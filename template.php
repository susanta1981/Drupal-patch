...
function bartik_process_page(&$variables) {
  ...
  if($variables['is_front']) {
    drupal_add_css(drupal_get_path('theme', 'bartik') . '/css/bootstrap.min.css');
    drupal_add_css(drupal_get_path('theme', 'bartik') . '/css/custom.css');
...
function bartik_form_alter(&$form, &$form_state, $form_id) {
  ...
  if($form_id == 'user_login') {
	...
	$form['name']['#description'] = '<p class="inputDescription margin-bottom-0"><em>Enter your '. $site_name .' username</em></p>';
...
function bartik_links__system_main_menu($variables) {
  ...
  $html = "<div class='menulist'><a href='#' class='close'></a>";
  $menu_list = menu_tree_all_data('menu-my-navigation');

  $arr = array_chunk($menu_list, 3);

  foreach($arr as $key => $value) {    
	$html .= '<div class="navlist-'.($key+1).'">';
	
	foreach($value as $menu_link) {	  
	  $link = $menu_link['link']['link_path'] == '<front>' ? l($menu_link['link']['link_title'], $site_frontpage) : l($menu_link['link']['link_title'], $menu_link['link']['link_path']);
	  $html .= '<h3>'. $link .'</h3>';
	  if(isset($menu_link['below']) && !empty($menu_link['below'])) {
	    $html .= '<div class="navlist-item"><ul>';
		foreach($menu_link['below'] as $child) {
		  $c_link = $child['link']['link_path'] == '<front>' ? l($child['link']['link_title'], $site_frontpage) : l($child['link']['link_title'], $child['link']['link_path']);
		  $html .= '<li>'. $c_link .'</li>';
		}
		$html .= '</ul></div>';
	  }
